...
const statusProgramaOptions = ['Diamond', 'Platinum', 'Gold', 'Silver'];

export default function Latam() {
  const { usuario } = useAuth();
  const [data, setData] = useState<LatamMembro[]>([]);
  ...
  const [editing, setEditing] = useState<LatamMembro | null>(null);
  ...
  const loadInitialData = async () => {
    try {
      ...
      const { data: programa } = await supabase
        .from('programas')
        .select('id')
        .eq('nome_programa', 'LATAM')
        .single();

      if (programa) {
        setProgramaId(programa.id);
        await Promise.all([
          loadData(programa.id),
          loadParceiros(),
          loadProdutos(),
          loadCartoes()
        ]);
  ...
  const loadCartoes = async () => {
    const { data } = await supabase.from('cartoes_credito').select('id, cartao').order('cartao');
    setCartoes(data || []);
  };

  const handleEdit = (item: LatamMembro) => {
    setEditing(item);
    setFormData({
  ...
  const handleDelete = async (item: LatamMembro) => {
    setDialogConfig({
      isOpen: true,
      type: 'warning',
      title: 'Confirmar Exclusao',
      message: `Deseja realmente excluir o membro Latam "${item.numero_fidelidade}"? Esta acao nao pode ser desfeita.`,
      onConfirm: async () => {
        try {
          ...
          await supabase.from('logs').insert({
            ...
            acao: 'DELETE',
            linha_afetada: `Latam - ${item.numero_fidelidade}`,
            dados_antes: item,
            dados_depois: null
          });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!programaId) {
      setDialogConfig({
        ...
        type: 'error',
        title: 'Erro',
        message: 'Programa Latam nao encontrado'
      });
      return;
    }
    ...
    try {
      if (editing) {
        ...
        await supabase.from('logs').insert({
          ...
          usuario_nome: usuario?.nome || '',
          acao: 'UPDATE',
          linha_afetada: `Latam - ${formData.numero_fidelidade}`,
          dados_antes: editing,
          dados_depois: data
        });
      } else {
  ...
  const columns = [
    {
      ...
      label: 'ID Parceiro',
      sortable: true,
      render: (row: LatamMembro) => row.parceiro?.id_parceiro || '-'
    },
    {
      key: 'parceiro.nome_parceiro',
      label: 'Nome Parceiro',
      sortable: true,
      render: (row: LatamMembro) => row.parceiro?.nome_parceiro || '-'
    },
    {
      key: 'parceiro.telefone',
      label: 'Telefone',
      sortable: false,
      render: (row: LatamMembro) => row.parceiro?.telefone || '-'
    },
    {
      key: 'parceiro.dt_nasc',
      label: 'Dt. Nasc.',
      sortable: false,
      render: (row: LatamMembro) => formatDate(row.parceiro?.dt_nasc)
    },
    {
      key: 'parceiro.cpf',
      label: 'CPF',
      sortable: false,
      render: (row: LatamMembro) => formatCPF(row.parceiro?.cpf)
    },
    {
      key: 'idade',
      label: 'Idade',
      sortable: false,
      render: (row: LatamMembro) => calcularIdade(row.parceiro?.dt_nasc || null)
    },
    { key: 'numero_fidelidade', label: 'Nº Fidelidade', sortable: true },
    {
      ...
      label: 'Clube',
      sortable: false,
      render: (row: LatamMembro) => row.clube_produto?.nome || '-'
    },
    {
      key: 'cartao.cartao',
      label: 'Cartão',
      sortable: false,
      render: (row: LatamMembro) => row.cartao?.cartao || '-'
    },
    {
      key: 'valor',
      label: 'Valor',
      sortable: false,
      sumable: true,
      render: (row: LatamMembro) => formatCurrency(row.valor)
    },
  ...
  return (
    <div className="p-6">
      <CrudTable
        title="Latam - Membros do Programa"
        data={data}
        columns={columns}
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium mb-1">Mudança Clube</label>
              <input
                type="text"
