...
/*
  # Corrigir custo médio na transferência entre pessoas

  1. Problema
    - O trigger de DESTINO buscava o custo_medio da origem APÓS o trigger de ORIGEM já ter
      processado a saída dos pontos.
    - Se a saída zerava o saldo da Livelo, o custo_medio ficava 0 e o destino recebia
      pontos com valor errado.

  2. Solução
    - Unificar em uma única função que:
      1. Lê o custo_medio da origem ANTES de processar qualquer saída
      2. Processa a saída da origem
      3. Processa a entrada no destino usando o custo_medio capturado no passo 1

  3. Mudanças
    - Substituir os dois triggers separados por um único trigger que chama
      `processar_transferencia_pessoas_completa()`
    - A nova função garante a ordem correta das operações
*/

-- Função unificada que processa origem e destino em ordem correta
...
BEGIN
  IF (TG_OP = 'INSERT' AND LOWER(NEW.status) = 'concluído') OR
    ...
    PERFORM atualizar_estoque_pontos(
      ...
      NEW.quantidade,
      'Saída',
      0, -- calculado internamente pelo custo_medio
...
'Processa transferência entre pessoas garantindo que o custo_medio da origem
seja capturado ANTES da saída, evitando distorção no cálculo do destino.';

-- Remover triggers antigos separados
...
-- Criar trigger único que processa tudo em ordem correta
