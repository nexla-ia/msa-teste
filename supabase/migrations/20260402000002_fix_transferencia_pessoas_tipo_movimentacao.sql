/*
  # Fix transferência entre pessoas: tipo correto no histórico

  ## Problema
  processar_transferencia_pessoas_completa() chama atualizar_estoque_pontos()
  sem passar p_tipo_movimentacao, então as entradas no estoque_movimentacoes
  ficam com tipo = 'entrada'/'saida' genérico em vez de
  'transferencia_pessoas_entrada'/'transferencia_pessoas_saida'.

  ## Correção
  Passar p_tipo_movimentacao correto em todas as chamadas.
*/
...
-- Garantir que o trigger está criado e ativo
DROP TRIGGER IF EXISTS trigger_transferencia_pessoas_completa ON transferencia_pessoas;
